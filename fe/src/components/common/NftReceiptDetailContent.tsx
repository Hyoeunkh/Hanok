...
import { FiAlertCircle, FiCheck, FiCheckCircle, FiChevronLeft, FiClock, FiCopy, FiExternalLink } from 'react-icons/fi';
  ...
  return (
    <div className="flex flex-col gap-6 text-neutral-100">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl leading-tight font-bold text-warm">NFT 영수증</h1>
      </div>

      {statusConfig && (
        <div className={`flex items-center gap-3 rounded-2xl border px-5 py-4 ${statusConfig.className}`}>
          {nft.txStatus === 'COMPLETED' && <FiCheckCircle className="h-5 w-5 shrink-0" />}
          {nft.txStatus === 'PENDING' && <FiClock className="h-5 w-5 shrink-0" />}
          {nft.txStatus === 'FAILED' && <FiAlertCircle className="h-5 w-5 shrink-0" />}
      ...
      <div className="rounded-3xl border border-neutral-800 bg-surface-elevated p-6 shadow-[0_30px_80px_rgba(0,0,0,0.28)] sm:p-8">
        ...
        <div className="flex flex-col">
          <InfoRow label="상태">
            {statusConfig && (
              <span
                className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-semibold ${statusConfig.className}`}
              >
                {statusConfig.label}
              </span>
            )}
