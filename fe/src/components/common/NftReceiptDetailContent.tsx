...
const STATUS_META: Record<StatusKey, StatusMeta> = {
  COMPLETED: {
    ...
    icon: (
      <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-[3px] bg-[#c0392b] font-bold text-black border border-[#b19c6f] text-sm leading-none pt-[2px]">
        완
      </div>
    ),
  },
  PENDING: {
    ...
    chipClassName: 'text-[#e1d5b4]',
    icon: (
      <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-[3px] bg-[#f1c40f] font-bold text-black border border-[#b19c6f]">
  ...
  FAILED: {
    ...
    chipClassName: 'text-[#e1d5b4]',
    icon: (
      <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-[3px] bg-[#e74c3c] font-bold text-black border border-[#b19c6f]">
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="shrink-0 text-[#a07a50]/70 transition hover:text-[#e1d5b4]"
      aria-label="복사"
    >
    ...
    <div className="mx-auto flex max-w-[400px] flex-col gap-5">
      ...
      <div className="relative flex flex-col items-center">
        ...
        <div className="relative h-20 w-6 shrink-0">
          <div className="absolute left-0 h-full w-[6px] rounded-full bg-gradient-to-r from-[#7a1515] via-[#a02020] to-[#8b1a1a] shadow-[2px_0_4px_rgba(0,0,0,0.5),-1px_0_2px_rgba(200,100,100,0.15)]" style={{ transform: 'rotate(-1deg)' }} />
          <div className="absolute right-0 h-full w-[6px] rounded-full bg-gradient-to-r from-[#8b1a1a] via-[#a02020] to-[#7a1515] shadow-[-2px_0_4px_rgba(0,0,0,0.5),1px_0_2px_rgba(200,100,100,0.15)]" style={{ transform: 'rotate(1deg)' }} />
        </div>

      {/* ── 호패 본체 ── */}
      <div
        className="relative w-full overflow-hidden rounded-[20px] shadow-[0_4px_8px_rgba(0,0,0,0.3),0_16px_32px_-8px_rgba(0,0,0,0.5),0_32px_64px_-16px_rgba(0,0,0,0.6)]"
      >
        {/* 나무 텍스처 배경 */}
        ...
        <div className="relative px-7 pt-14 pb-8 sm:px-9 sm:pb-10">
          ...
          <div className="mb-4 text-center">
            ...
            <div className="mt-1 text-[20px] font-bold tracking-[0.4em] text-[#d4c4a0]" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>
              HANOK
            </div>
            <div className="text-[14px] font-bold tracking-[0.4em] text-[#d4c4a0]" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>
              NFT RECEIPT
            </div>
          </div>
          ...
          {/* 품명 */}
          <div className="mb-4 text-center">
            <div className="text-[12px] font-semibold tracking-[0.3em] text-[#d4c4a0]" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>
              품 명
            </div>
            <h3 className="mt-1 text-2xl font-bold uppercase leading-snug tracking-wider text-[#e1d5b4]/90" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.5), 0 -1px 1px rgba(225,213,180,0.2)' }}>
          ...
          <div className="mb-4 flex items-center gap-4 rounded-md border border-[#b19c6f]/60 bg-black/20 px-4 py-3.5">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded text-lg">
              {status.icon}
            </div>
            <div>
              <div className="text-[15px] font-bold text-white" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>
                {status.label}
              </div>
              <div className="mt-0.5 text-[13px] font-medium text-[#d4c4a0]" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>
                {status.headline}
              </div>
            </div>
          </div>
          ...
          <div className="mb-4 space-y-1.5 text-[14px]">
            {[
              { label: 'SELLER', value: nft.sellerNickname ?? '정보 없음' },
              { label: 'BUYER', value: nft.buyerNickname ?? '정보 없음' },
              { label: 'PRICE', value: nft.price != null ? formatPrice(nft.price) : '--' },
              { label: 'DATE / TIME', value: nft.mintedAt ? `${formatShortDate(nft.mintedAt)} ${formatTime(nft.mintedAt)}` : '--' },
            ].map((item) => (
              <div key={item.label} className="flex justify-between gap-4">
                <span className="font-semibold text-[#d4c4a0]" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>{item.label}</span>
                <span className="font-bold text-white" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>{item.value}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mb-1 grid grid-cols-3 gap-2">
            {[
              ...
              >
                <div className="text-[14px] font-bold text-[#e1d5b4]" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.6)' }}>
                  {item.value}
                </div>
              </div>
            ))}
          </div>
          <div className="mb-4 text-center text-[10px] text-[#a07a50]">
            NETWORK Sepolia / ERC-721 / TOKEN ID
          </div>

          {/* TX Hash */}
          <div className="mb-4 flex flex-col">
            <span className="text-[11px] text-[#a07a50]">TX HASH</span>
            <div className="mt-1 flex h-9 items-center justify-between rounded-sm border border-[#b19c6f] bg-black/15 px-3 shadow-inner">
              <span className="truncate text-[11px] font-medium text-[#e1d5b4]/80" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.6)' }}>
                {hashLabel}
              </span>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-[12px] font-bold tracking-[0.2em] text-[#e1d5b4]" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.5), 0 -1px 1px rgba(225,213,180,0.2)' }}>
              {barcodeDigits}
            </span>
          </div>
          ...
          <div className="mt-6 flex flex-col items-center">
            <img src={hanokLogo} alt="한옥" className="h-16 w-auto opacity-70" />
          </div>
        </div>
      </div>
      </div>
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row">
          {nft.txHash && nft.txStatus === 'COMPLETED' && (
            <a
              ...
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3.5 text-sm font-bold text-white shadow-[0_2px_8px_rgba(0,0,0,0.3)] transition hover:bg-primary-light"
            >
          ...
          {isModal && (
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl bg-neutral-700 px-5 py-3.5 text-sm font-bold text-white shadow-[0_2px_8px_rgba(0,0,0,0.3)] transition hover:bg-neutral-600 sm:min-w-[100px]"
            >
              닫기
            </button>
          )}
