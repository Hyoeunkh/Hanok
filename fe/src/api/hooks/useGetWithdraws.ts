...
import type { ApiResponse, GetWithdrawsResponse, WithdrawStatus } from '@/types';

export const getWithdrawsPath = () => `/v1/admin/withdraws`;

export const getWithdraws = async (status?: WithdrawStatus) => {
  const params = status ? { status } : {};
  const response = await getFetchInstance().get<ApiResponse<GetWithdrawsResponse>>(getWithdrawsPath(), { params });
